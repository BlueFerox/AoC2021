...
def solve(input : str):
    ...
    fields = [[[BingoNum(int(n), False) for n in line.split(' ')] for line in block.split('\n') if line != ''] for block in blocks[1:]]
            ...
            for line in field:
                for bn in line:
                    if bn.n == picked and (not bn.marked):
                        bn.marked = True
            if hasWon(field):
                return picked * sum(sum(bn.n for bn in line if not bn.marked) for line in field)
    return -1
